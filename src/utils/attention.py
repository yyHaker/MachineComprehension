...
""" some useful attention mechanism.
@author: yyhaker
...
from .fucntions import masked_flip, masked_softmax
...
class MatchRNN(nn.Module):
    ...
    Args:
        ...
        - enable_layer_norm: whether to use layer normlization
    Inputs:
    ...
    def forward(self, Hp, Hp_mask, Hq, Hq_mask):
        ...
        # [batch, context_len]  --> [context_len, batch, 1]
        Hp_mask = Hp_mask.transpose(0, 1).unsqueeze(2)
        # print("Hp_mask size:", Hp_mask.size())
        # print("rtn_hidden size: ", rtn_hidden.size())

        real_rtn_hidden = Hp_mask * rtn_hidden
...
class SelfAttention(nn.Module):
    ...
    def forward(self, encoder_outputs):
        ...
        # weight: (BxL)
        weight = F.softmax(energy.squeeze(-1), dim=-1)
        # (BxLxD) * (BxLx1) -> (BxD)
