...
class BiDAFMultiParasNewPR(BiDAFMultiParas):
    '''Try anathoer attention for PR according to 18MRC竞赛第一名分享'''
    def __init__(self, args, pretrained, trainable_weight_idx):
        super(BiDAFMultiParasNewPR, self).__init__(args, pretrained, trainable_weight_idx)
        self.score_weight_qp = self.align_weight_p = self.align_weight_p = None
        # New Attention
        self.W_pr = Linear(self.args["hidden_size"] * 2, self.args["hidden_size"])
        self.V_1 = Linear(self.args["hidden_size"], 1, bias=False)
        self.V_sp = Linear(self.args["hidden_size"] * 2, 1, bias=False)
    ...
    def forward(self, input_data, train=True):
        ...
        # reshape_m = m.view(batch_size, max_para_num, max_para_len, -1)
        # self.logger.info(f'reshape_m:{m.shape}')
        S = self.V_1(F.tanh(self.W_pr(m)))  # m: (b ,max_para_num, max_p_len, 2*d) --->  m: (b, max_para_num, max_p_len, d)  --->  m: (b, max_para_num, max_p_len, 1)
        reshape_paras_word_mask = paras_word_mask.view(batch_size*max_para_num, -1).unsqueeze(2)
        # self.logger.info(f'S:{S.shape}, mask:{reshape_paras_word_mask.shape}')
        A = softmax_mask(S, mask=reshape_paras_word_mask, dim=1)  # (b, max_para_num, max_p_len, 1)
        # self.logger.info(f'A:{A.shape}')
        R = torch.sum(A * m, dim=1)  # (b, max_para_num, 2d)
        # self.logger.info(f'R:{R.shape}')
        G = self.V_sp(R)  # (b, max_para_num, 1)
        G = G.view(batch_size, -1)
        # self.logger.info(f'G:{G.shape}')
        G = -INF * (1 - paras_mask) + G  # Applay mask
